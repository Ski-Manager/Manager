...
Class Facebook
{
    ...
    public function __construct(){
        ...
        if ($this->config->item('facebook_auth_on_load') === TRUE){
            ...
            $this->authenticate('login_controller/checkLogin');
        }
    }
     ...
     */
    public function is_authenticated($from_url){
        $access_token = $this->authenticate($from_url);
    ...
    public function login_url_register(){
        // Login type must be web, else return empty string
        if($this->config->item('facebook_login_type') != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . 'register_controller',
            $this->config->item('facebook_permissions')
        );
    }
    public function login_url_reset_account(){
        // Login type must be web, else return empty string
        if($this->config->item('facebook_login_type') != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . 'reset_controller/proceed_reset_account',
            $this->config->item('facebook_permissions')
        );
    }
    public function login_url_delete_account(){
        // Login type must be web, else return empty string
        if($this->config->item('facebook_login_type') != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . 'reset_controller/proceed_delete_account',
            $this->config->item('facebook_permissions')
        );
    }
     ...
     */
    private function authenticate($from_url){
        ...
        if(!$access_token){
            try{
                $access_token = $this->helper->getAccessToken(base_url().$from_url);
            }catch (FacebookSDKException $e){
