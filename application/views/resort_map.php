<div class="w-full">
    ...
    echo '<div>';
        echo '<h2 class="h2">'.$this->lang->line('resort_map')['resort_map_title'].'</h2>';
    echo '</div>';
    ...
    echo '<div class="md:col-span-12 padding15-no-top">';
        ...
        echo '<h4 class="h4">'.$this->lang->line('resort_map')['to_build_slope_title'].'</h4>';
        echo $this->lang->line('resort_map')['to_build_slope_instructions'];
    ...
    echo '<div class="min_height">';
        ...
        echo '<div class="md:col-span-12">';
            // slopes   
            echo '<div class="join-vertical col-md-1" role="group">';
            echo '<button type="button" class="btn btn-secondary button_menu '.$build_slope_button_status.'" id="build_slope">'.$this->lang->line('home')['big_slopes'].'</button></div>';
            echo '<div id="slope_type_button" class="join-vertical md:col-span-3"></div>';
            echo '<div id="slope_difficulty_button" class="join-vertical md:col-span-3"></div>';
            echo '<div id="slope_table_info" class="col-md-5"></div>';
        echo '</div>';
    echo '</div>'; 
        ...
        echo '<div class="md:col-span-8">';        
            ...
            echo form_hidden('form_id_slope', '');
            echo form_hidden('form_id_slope_type_from_page', '');
            echo form_hidden('form_difficulty', '');
    ...
    Settings.show_sectors = '<?php echo $this->lang->line('resort')['show_sectors']; ?>';
    Settings.accessible_sectors = <?php echo json_encode(array_values(array_filter($accessible_sectors, function($s){ return $s !== '' && $s !== null; }))); ?>;
    <?php if (!empty($map_type)): ?>
    Settings.map_image_url  = '<?php echo base_url($map_type->map_image); ?>';
    Settings.map_center_x   = <?php echo (int)$map_type->default_center_x; ?>;
    Settings.map_center_y   = <?php echo (int)$map_type->default_center_y; ?>;
    Settings.map_zoom       = <?php echo (int)$map_type->default_zoom; ?>;
    <?php endif; ?>
    </script>
