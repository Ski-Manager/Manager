 ...
 */
class Migration_Add_resort_facilities extends CI_Migration {
    ...
    public function up() {
        // Create the per-resort facility revenue statistics table
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `game_resort_rev_facility` (
                `id`          INT(11)        NOT NULL AUTO_INCREMENT,
                `id_resort`   INT(11)        NOT NULL,
                `date`        DATE           NOT NULL,
                `rev_facility` DECIMAL(15,2) NOT NULL DEFAULT '0',
                PRIMARY KEY (`id`),
                KEY `idx_rev_facility_resort_date` (`id_resort`, `date`)
    ...
    public function down() {
        $this->db->query("DROP TABLE IF EXISTS `game_resort_rev_facility`");
        $this->db
            ->where('type', 'facility')
