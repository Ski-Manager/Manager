 ...
 * 
 */
class Snow_cannon_controller extends CI_Controller{
...
//LINE ABOVE TO EDIT 
        ...
        if ($user_activated) {      // If the account is activated, we show the page
            ...
            if ($checkIfResortExists->num_rows() > 0) {                                        // if the player has a resort, OK
                ...
                $building_ach_uncloked = $this->building_model->get_building_ach_unlocked2($currentUserID, '30');   // Checks if the player has uncloked the achievement "build one medical center" (ID 30)
                $achievement_data = $this->achievements_model->get_specific_achievements_data(30);   // get generic achievement info
                ...
                $ach_name = $info_achievement_data->$column_name;
     ...
     * @param type $building_type            ID type of building (3, 4)
    ...
    public function standardBuildingBlock($building_type, $currentResortID){
        ...
        for ($i=1;$i<=3;$i++) {
            // Gets the generic data for the building type and level
            $building_data = $this->building_model->get_generic_building_data($building_type, $i);    // Type = hotel, restaurants... and "i" for each level ($i = 1, 2, 3)
            if ($building_data->num_rows() > 0) {                // the generic building exists in the DB (always!)
                ...
                if ($count_this_building_level_under_construction == '1'){              // If the current level is under construction
                    ...
                    if ($time_left_value <= '0'){                                   // If there is no time left (building finished)
                        ...
                        $data['pre_building_time'][$i] = '<a href="'.base_url().'snow_cannon_controller/"><div class="tooltip tooltip-bottom" data-tip="'.$this->lang->line('home')['wait_tooltip'].'">';   // For toolpit (pre)
                ...
                $data['buildingMaxIncome'][$i] = number_format($building_dataArray->daily_cost, 0, ',', ' ');
                $data['buildingCapacity'][$i] = number_format($building_dataArray->capacity, 0, ',', ' ');
                ...
                if ( isset($time_left_value) && $time_left_value > 0 ) {
                    ...
                    if ($genepis_required_to_rush <= $genepis_available) {
                        $button_level[$i] = '<div style="display:inline;" class="tooltip tooltip-left" data-tip="'.$this->lang->line('home')['speed_up_for_genepis'].'"><a href="'.base_url('snow_'.$building_type.'_controller/rush/'.$building_type.'/'.$i).'"><button class="btn btn-success">'.$this->lang->line('home')['rush'].' '.$this->lang->line('home')['for'].' '.$genepis_required_to_rush.' '.$this->lang->line('home')['genepis_title'].'</button></a></div>';
                    }
                    else {
                ...
                if ($count_this_building_under_construction == 0) { // If the building is not under construction nor upgrade
                    if ($i == '1'){         // for the first level, we allow the BUILD button
                        $data['buildingButton'][$i] = '<td><a href="'.base_url().'snow_cannon_controller/build_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['build'].'</button></a></td>';
                    }
                      ...
                      if ($count_num_building_previous_level == '0') {      // If the previous one is NOT built, we DISABLE upgrade
                            ...
                            $data['buildingButton'][$i] = '<td><a href="'.base_url().'snow_cannon_controller/upgrade_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['upgrade'].'</button></a></td>';  
                        }  
                    }
                }
        ...
        // counts how many buildings are built in total (any level, status)
        $open_cannon = $this->building_model->count_this_building_level_db($currentResortID, $building_type, '', '1', 'both', 'none');
        $closed_cannon = $this->building_model->count_this_building_level_db($currentResortID, $building_type, '', '2', 'both', 'none');
        $data['active_cannons'] = '<p>'.$this->lang->line('building')['there_are'].$open_cannon.$this->lang->line('building')['active_text_cannon'].'</p>';
        $data['inactive_cannons'] = '<p>'.$this->lang->line('building')['there_are'].$closed_cannon.$this->lang->line('building')['inactive_text_cannon'].'</p>';
        // We only show the OPEN button if there is at least one closed cannon
        if ($closed_cannon > 0)
        $data['start_cannon_button'] = '<a href="'.base_url().'snow_cannon_controller/change_state_all/'.$currentResortID.'/1"><button class="btn btn-success">'.$this->lang->line('building')['start_all'].'</button></a>';
        else
            $data['start_cannon_button'] = '';
        // We only show the CLOSE button if there is at least one open cannon
        if ($open_cannon > 0)
            $data['stop_cannon_button'] = '<a href="'.base_url().'snow_cannon_controller/change_state_all/'.$currentResortID.'/2"><button class="btn btn-danger">'.$this->lang->line('building')['stop_all'].'</button></a>';
        else
            $data['stop_cannon_button'] = '';
        
        $friendly_status = 'closed';
        $data['cannonBuildingStatus'] = $this->lang->line('home')['building_status_to_show_'.$friendly_status.''];
        ...
        $data['cannonBuildingStatus_real'] = $class.$this->lang->line('home')['building_status_to_show_'.$friendly_status.''].'</span>';

        // Current snow level in resort
        ...
        // Today's temperature for snow quality
        ...
        foreach ($cannons_data->result() as $cannon) {
            if ($cannon->id_status == '1') {
                $status_label = $this->lang->line('building')['cannon_status_active'];
                $status_class = 'alert-success';
                $toggle_button = '<a href="'.base_url().'snow_cannon_controller/change_state_single/'.$currentResortID.'/'.$cannon->id_created_buildings.'/2"><button class="btn btn-danger btn-xs">'.$this->lang->line('building')['stop_cannon'].'</button></a>';
                $total_snow_output += (int)$cannon->capacity;
                $total_daily_cost  += (int)$cannon->daily_cost;
                $active_cannon_count++;
            } elseif ($cannon->id_status == '4') {
                $status_label = $this->lang->line('building')['cannon_status_construction'];
                $status_class = 'alert-warning';
                $toggle_button = '';
            } else {
                $status_label = $this->lang->line('building')['cannon_status_inactive'];
                $status_class = 'alert-danger';
                $toggle_button = '<a href="'.base_url().'snow_cannon_controller/change_state_single/'.$currentResortID.'/'.$cannon->id_created_buildings.'/1"><button class="btn btn-success btn-xs">'.$this->lang->line('building')['start_cannon'].'</button></a>';
            }
            $cannon_list[] = [
                'number'       => $cannon_number,
                'level'        => $cannon->level,
                'capacity'     => $cannon->capacity,
                'daily_cost'   => number_format($cannon->daily_cost, 0, ',', ' '),
        ...
        $data['cannon_list'] = $cannon_list;

        // Snow cannon production summary
        $data['total_snow_output'] = $total_snow_output;
        $data['total_daily_cost_cannons'] = number_format($total_daily_cost, 0, ',', ' ');

        // Snow level progress bar (0–MAX_SNOW_LEVEL cm scale)
        $snow_level = $data['current_snow_level'];
        $snow_max = MAX_SNOW_LEVEL;
        $snow_percent = min(100, round($snow_level / $snow_max * 100));
        if ($snow_percent >= 60)
        ...
        // Snow target level
        $cannon_target = $this->resort_model->get_cannon_target_snow_DB($currentResortID);
        $data['cannon_target_snow'] = $cannon_target;
        $data['snow_target_form'] = '
            <form method="post" action="'.base_url().'snow_cannon_controller/set_snow_target/'.$currentResortID.'">
                <div class="input-group" style="max-width:280px;">
                    <input type="number" name="target_snow" class="form-control" min="0" max="'.MAX_SNOW_LEVEL.'" value="'.$cannon_target.'" placeholder="0 = '.$this->lang->line('building')['snow_target_disabled'].'">
        ...
        $cannon_auto_start = $this->resort_model->get_cannon_auto_start_DB($currentResortID);
        $data['cannon_auto_start'] = $cannon_auto_start;
        $data['snow_auto_start_form'] = '
            <form method="post" action="'.base_url().'snow_cannon_controller/set_snow_auto_start/'.$currentResortID.'">
                <div class="input-group" style="max-width:280px;">
                    <input type="number" name="auto_start_snow" class="form-control" min="0" max="'.MAX_SNOW_LEVEL.'" value="'.$cannon_auto_start.'" placeholder="0 = '.$this->lang->line('building')['snow_auto_start_disabled'].'">
                    <span class="input-group-text">'.$this->lang->line('building')['cm'].'</span>
                    <button type="submit" class="btn btn-primary">'.$this->lang->line('building')['save_snow_auto_start'].'</button>
                </div>
                <small class="text-muted">'.$this->lang->line('building')['snow_auto_start_info'].'</small>
            </form>';

        // Snow level history (last SNOW_HISTORY_DAYS days)
        $graph_start_date = gmdate('Y-m-d', strtotime('-'.SNOW_HISTORY_DAYS.' days'));
        $snow_history_data = $this->finances_model->get_history_for_graph($currentResortID, 'snow_level', $graph_start_date);
        $data['snow_level_history'] = $snow_history_data->result();

        // Water reservoir level
        $data['water_reservoir_purchased'] = $this->resort_model->get_water_reservoir_purchased_DB($currentResortID);
        ...
        // Snowmaking staff count
        $snowmaker_count = $this->staff_model->count_hired_snowmakers_DB($currentResortID);
        $data['snowmaker_count'] = $snowmaker_count;
        $data['snowmaker_required'] = SNOWMAKING_MIN_STAFF;

        // Tonight's temperature status (above/below freezing)
        $todays_date = gmdate('Y-m-d');
        $todays_weather = $this->weather_model->select_weather_forecast($todays_date);
        $above_freezing = false;
        if ($todays_weather !== false && $todays_weather->num_rows() > 0) {
            $weather_row = $todays_weather->row();
            $conditions  = $this->weather_model->select_weather_conditions($weather_row->id_condition);
            if ($conditions->num_rows() > 0) {
                $cond = $conditions->row();
                $above_freezing = ((int)$cond->snow_level < 0);
            }
        }
        $data['above_freezing'] = $above_freezing;

        // Electricity costs info
        $data['snowmaking_elec_per_cannon'] = SNOWMAKING_ELECTRICITY_PER_CANNON;
        $data['snowmaking_elec_per_trail']  = SNOWMAKING_ELECTRICITY_PER_TRAIL;
    ...
    /**
     * set_snow_auto_start  Sets the cannon auto-start threshold for a resort
     *
     * @param type $currentResortID   Current resort ID
     */
    public function set_snow_auto_start($currentResortID){
        ...
        $threshold = (int)$this->input->post('auto_start_snow');
        $threshold = max(0, min($threshold, MAX_SNOW_LEVEL));
        $result = $this->resort_model->set_cannon_auto_start_DB($currentResortID, $threshold);
        if ($result !== false) {
            $data['infoMessage'] = 'snow_auto_start_saved';
        } else {
    ...
    /**
     * set_snow_target      Sets the cannon snow target level for a resort
     *
     * @param type $currentResortID   Current resort ID
     */
    public function set_snow_target($currentResortID){
        ...
        if ($result !== false) {
            $data['infoMessage'] = 'snow_target_saved';
        } else {
            $data['infoMessage'] = 'bad_action';
        }
        $this->index($data);
        ...
        if(!$pageWasRefreshed ) {
            if ($count_this_building_under_construction == 0) {                         // If not under construction for this building, we are allowed to building more
                ...
                if ($money_after_payment >= 0) {                                         // If enough cash
                    ...
                    $build_building = $this->building_model->build_building_db($data1);   // DB request
                    if ($build_building) {
                        ...
                        $call_achievements_check = call_achievements_check($data2 = array('id_sector' => '*', 'id_resort' => $currentResortID, 'quantity' => $cost_building), 'build_amount');   // Check spending achievements
            ...
            if ($count_num_building_previous_level >= '1') {
                if ($count_this_building_under_construction == 0) {                     // If not under construction for this building, we are allowed to upgrade
                    ...
                    if ($money_after_payment >= 0) {                                    // If enough cash
                        ...
                        $build_building = $this->building_model->update_building_db($currentResortID, $building_type, $level-1 , $data1);    // Build request in the DB
                        if ($build_building) {
                            ...
                            $call_achievements_check = call_achievements_check($data2, 'upgrade');   // Builds the building in the DB
    ...
    /**
     * change_state_all                Opens/closes all cannons in the database
     * @param type $currentResortID   Current resort ID
     */
    public function change_state_all($currentResortID, $action_to_do){
        // START CHECK IF THE USER IS ALLOWED TO BE HERE
        $user_is_allowed = $this->users_model->check_current_user_allowed($currentResortID);
        if (!$user_is_allowed)
            redirect('home_controller');
        ...
        $is_the_building_built = $this->building_model->get_created_buildings_for_player($currentResortID, 'cannon');     // check if building is built for this player
        if (($is_the_building_built->num_rows() > 0) && ($action_to_do == '1' || $action_to_do == '2')){           // building is built, let's continue           
            $data = array (
                'id_status' => $action_to_do       // $action_to_do = 1 (open) or 2 (close) (id_status)
            );   
            $result = $this->building_model->update_open_close_building_db($currentResortID, 'cannon', $data);    // Opens/closes the cannons in the database
            if ($result) {
                if ($action_to_do == '1'){
                    $data['infoMessage'] = 'cannons_started';
                }
                else if ($action_to_do == '2'){
                    $data['infoMessage'] = 'cannons_stopped';
                }
                $this->index($data);
            }
            ...
            $data['infoMessage'] = 'building_not_existing';
                $this->index($data);
        }
    }
    ...
    /**
     * change_state_single          Opens or closes a single cannon
     * @param type $currentResortID         Current resort ID
     * @param type $id_created_buildings    ID of the cannon to update
     * @param type $action_to_do            1 = open, 2 = close
     */
    public function change_state_single($currentResortID, $id_created_buildings, $action_to_do){
        // START CHECK IF THE USER IS ALLOWED TO BE HERE
        $user_is_allowed = $this->users_model->check_current_user_allowed($currentResortID);
        if (!$user_is_allowed)
            redirect('home_controller');
        ...
        if ($action_to_do == '1' || $action_to_do == '2') {
            $data = [
                'id_status' => $action_to_do
            ];
            $result = $this->building_model->update_single_cannon_status_db($currentResortID, $id_created_buildings, $data);
            if ($result) {
                $data['infoMessage'] = ($action_to_do == '1') ? 'cannon_started' : 'cannon_stopped';
            } else {
                $data['infoMessage'] = 'bad_action';
            }
        } else {
