...
/**
 * Marketing_upgrade_controller
 *
 * Manages the Marketing Upgrade Research Tree.
 * Players research marketing improvements that take time and money to complete.
 */
class Marketing_upgrade_controller extends CI_Controller {
    ...
    public function start_research($currentResortID, $upgrade_key) {
        ...
        if (!array_key_exists($upgrade_key, MARKETING_UPGRADE_TREE)) {
            $this->session->set_flashdata('infoMessage', 'bad_action');
            $this->session->set_flashdata('message_tree', 'marketing_upgrade');
            redirect('upgrades_controller');
        }

        $info = MARKETING_UPGRADE_TREE[$upgrade_key];

        if (!is_null($info['prerequisite'])) {
            $prereq_row  = $this->marketing_upgrade_model->get_research_row_DB($currentResortID, $info['prerequisite']);
            $prereq_done = false;
            if ($prereq_row) {
        ...
        $existing = $this->marketing_upgrade_model->get_research_row_DB($currentResortID, $upgrade_key);
        if ($existing) {
        ...
        $result = $this->marketing_upgrade_model->start_research_DB($currentResortID, $upgrade_key, $info['duration_days']);
        ...
        if ($result) {
            ...
            $this->logs_model->call_notification_DB([
                ...
                'type'      => $this->lang->line('marketing_upgrade')['log_type'],
                'data'      => $this->lang->line('marketing_upgrade')['log_started'] . ': ' . $upgrade_name,
            ]);
            log_user_action([
                'id_player' => $currentUserID,
                'type'      => $this->lang->line('marketing_upgrade')['log_type'],
