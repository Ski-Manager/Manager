...
//LINE ABOVE TO EDIT 
        ...
        if ($user_activated) {      // If the account is activated, we show the page
            ...
            if ($checkIfResortExists->num_rows() > 0) {                                        // if the player has a resort, OK
                ...
                // If tourist info center built, we can display the page
    ...
    public function standardBuildingBlock($building_type, $currentResortID){
        ...
        // Ensure the facility building rows exist in game_buildings before querying them
        $this->ensure_facility_buildings_exist();

        // Initialize arrays with default values for all 3 levels to prevent undefined array key warnings
        $data['buildingName'] = [1 => '', 2 => '', 3 => ''];
        $data['buildingCost'] = [1 => '', 2 => '', 3 => ''];
        $data['buildingTime'] = [1 => '', 2 => '', 3 => ''];
        $data['buildingButton'] = [1 => '', 2 => '', 3 => ''];
        $data['buildingReputation'] = [1 => '', 2 => '', 3 => ''];
        $data['buildingMaxIncome'] = [1 => '', 2 => '', 3 => ''];
        $data['buildingQuantity'] = [1 => '', 2 => '', 3 => ''];
        $data['pre_building_time'] = [1 => '', 2 => '', 3 => ''];
        $data['post_building_time'] = [1 => '', 2 => '', 3 => ''];

        // For each of the three levels
        for ($i=1;$i<=3;$i++) {
            // Gets the generic data for the building type and level
            $building_data = $this->building_model->get_generic_building_data($building_type, $i);    // Type and "i" for each level ($i = 1, 2, 3)
            if ($building_data->num_rows() > 0) {                // the generic building exists in the DB (always!)
                $building_dataArray = $building_data->row();
                // initializes variables in case we don't go into the right loop
                ...
                if ($count_this_building_level_under_construction == '1'){              // If the current level is under construction
                    ...
                    if ($time_left_value <= '0'){                                   // If there is no time left (building finished)
                        ...
                        $data['pre_building_time'][$i] = '<a href="'.base_url().'facilities_controller/"><div class="tooltip tooltip-bottom" data-tip="'.$this->lang->line('home')['wait_tooltip'].'">';   // For tooltip (pre)
                        $data['post_building_time'][$i] = '</div></a>';   // For tooltip (post)
                ...
                if ( isset($time_left_value) && $time_left_value > 0 ) {
                    ...
                    if ($genepis_required_to_rush <= $genepis_available) {
                        $button_level[$i] = '<div style="display:inline;" class="tooltip tooltip-left" data-tip="'.$this->lang->line('home')['speed_up_for_genepis'].'"><a href="'.base_url('facilities_controller/rush/'.$building_type.'/'.$i).'"><button class="btn btn-success">'.$this->lang->line('home')['rush'].' '.$this->lang->line('home')['for'].' '.$genepis_required_to_rush.' '.$this->lang->line('home')['genepis_title'].'</button></a></div>';
                    }
                    else {
                ...
                if ($count_this_building_under_construction == 0) { // If the building is not under construction nor upgrade
                    if ($i == '1'){         // for the first level, we allow the BUILD button
                        $data['buildingButton'][$i] = '<td><a href="'.base_url().'facilities_controller/build_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['build'].'</button></a></td>';
                    }
                      ...
                      if ($count_num_building_previous_level == '0') {      // If the previous one is NOT built, we DISABLE upgrade
                            ...
                            $data['buildingButton'][$i] = '<td><a href="'.base_url().'facilities_controller/upgrade_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['upgrade'].'</button></a></td>';  
                        }  
                    }
                }
     ...
     * 
     * @param type $currentResortID   Current resort ID
     * @param type $building_type            Type of the building
    ...
    public function build_building($currentResortID, $building_type, $level = '1'){
        ...
        if(!$pageWasRefreshed ) {
            if ($count_this_building_under_construction == 0) {                         // If not under construction for this building, we are allowed to build more
                ...
                if ($money_after_payment >= 0) {                                         // If enough cash
                    if ($removeCashQuery = $this->users_model->pay_item($cost_building, $cash_player)){      //the payment for the building has been taken
                    ...
                    $build_building = $this->building_model->build_building_db($data);   // DB request
                    if ($build_building) {
                        ...
                        $data_achievement = array (
                            ...
                            'id_resort' => $currentResortID,
                            'id_building' => $id_building,       // ID of the building in the game_buildings table
                            'type' => $building_type,       // type of building
                            'level' => '1',
                        );
            ...
            if ($count_num_building_previous_level >= '1') {
                if ($count_this_building_under_construction == 0) {                     // If not under construction for this building, we are allowed to upgrade
                    ...
                    if ($money_after_payment >= 0) {                                    // If enough cash
                        ...
                        $build_building = $this->building_model->update_building_db($currentResortID, $building_type, $level-1 , $data);    // Build request in the DB
                        if ($build_building) {
    ...
    /**
     * ensure_facility_buildings_exist  Inserts the resort facility catalogue rows into
     *                                   game_buildings if they are not already present.
     *                                   This removes the dependency on the migration system
     *                                   being enabled.
     */
    private function ensure_facility_buildings_exist() {
        $levels = [
            [
                'level'         => 1,
