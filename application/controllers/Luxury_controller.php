 ...
 * 
 */
class Luxury_controller extends CI_Controller{
...
//LINE ABOVE TO EDIT 
        ...
        if ($user_activated) {      // If the account is activated, we show the page
            ...
            if ($checkIfResortExists->num_rows() > 0) {                                        // if the player has a resort, OK
                ...
                // If tourist info center is built, we can display the page
    ...
    public function standardBuildingBlock($building_type, $currentResortID){
        ...
        // Ensure the luxury building rows exist in game_buildings before querying them
        $this->ensure_luxury_buildings_exist();

        // For each of the three levels
        for ($i=1;$i<=3;$i++) {
            // Gets the generic data for the building type and level
            $building_data = $this->building_model->get_generic_building_data($building_type, $i);    // Type (otel, restaurants...) and "i" for each level ($i = 1, 2, 3)
            if ($building_data->num_rows() > 0) {                // the generic building exists in the DB (always!)
                ...
                if ($count_this_building_level_under_construction == '1'){              // If the current level is under construction
                    ...
                    if ($time_left_value <= '0'){                                   // If there is no time left (building finished)
                        ...
                        $data['pre_building_time'][$i] = '<a href="'.base_url().'luxury_controller/"><div class="tooltip tooltip-bottom" data-tip="'.$this->lang->line('home')['wait_tooltip'].'">';   // For toolpit (pre)
                ...
                if ($count_this_building_under_construction == 0) { // If the building is not under construction nor upgrade
                    if ($i == '1'){         // for the first level, we allow the BUILD button
                        $data['buildingButton'][$i] = '<td><a href="'.base_url().'luxury_controller/build_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['build'].'</button></a></td>';
                    }
                      ...
                      if ($count_num_building_previous_level == '0') {      // If the previous one is NOT built, we DISABLE upgrade
                            ...
                            $data['buildingButton'][$i] = '<td><a href="'.base_url().'luxury_controller/upgrade_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['upgrade'].'</button></a></td>';  
                        }  
                    }
                }
    ...
    /**
     * ensure_luxury_buildings_exist    Inserts the luxury building catalogue rows into
     *                                   game_buildings if they are not already present.
     *                                   This removes the dependency on the migration system
     *                                   being enabled.
     */
    private function ensure_luxury_buildings_exist() {
        $levels = [
            [
                'level'         => 1,
