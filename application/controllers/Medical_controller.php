 ...
 * 
 */
class Medical_controller extends CI_Controller{
...
//LINE ABOVE TO EDIT 
    ...
    public function standardBuildingBlock($building_type, $currentResortID){
        ...
        $capacity_per_level = [];   // tracked per level to compute post-loop summary
        $income_per_level = [];     // tracked per level to compute post-loop summary
            ...
            if ($building_data->num_rows() > 0) {                // the generic building exists in the DB (always!)
                ...
                if ($count_this_building_level_under_construction == '1'){              // If the current level is under construction
                    ...
                    if ($time_left_value <= '0'){                                   // If there is no time left (building finished)
                        ...
                        $data['pre_building_time'][$i] = '<a href="'.base_url().'medical_controller/"><div class="tooltip tooltip-bottom" data-tip="'.$this->lang->line('home')['wait_tooltip'].'">';   // For toolpit (pre)
                ...
                $capacity_per_level[$i] = $building_dataArray->capacity;
                $default_max_income = $building_dataArray->max_income;
                $income_per_level[$i] = $default_max_income;
                ...
                if ($count_this_building_under_construction == 0) { // If the building is not under construction nor upgrade
                    if ($i == '1'){         // for the first level, we allow the BUILD button
                        $data['buildingButton'][$i] = '<td><a href="'.base_url().'medical_controller/build_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['build'].'</button></a></td>';
                    }
                      ...
                      if ($count_num_building_previous_level == '0') {      // If the previous one is NOT built, we DISABLE upgrade
                            ...
                            $data['buildingButton'][$i] = '<td><a href="'.base_url().'medical_controller/upgrade_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['upgrade'].'</button></a></td>';  
                        }  
                    }
                }
        ...
        // Compute summary stats for built medical buildings (not under construction)
        $total_capacity = 0;
        $total_max_income = 0;
        $total_count = 0;
        for ($j=1;$j<=3;$j++) {
            if (!isset($capacity_per_level[$j], $data['buildingQuantity'][$j])) continue;
            $count_built = $data['buildingQuantity'][$j] - count_this_building_level($building_type, $j, '4');
            if ($count_built > 0) {
                $total_capacity += $capacity_per_level[$j] * $count_built;
                $total_max_income += $income_per_level[$j] * $prestige_bonus['coef'] * $count_built;
                $total_count += $count_built;
            }
        }
        if ($total_count > 0) {
            $data['medicalTotalCount'] = $total_count;
            $data['medicalTotalCapacity'] = number_format($total_capacity, 0, ',', ' ');
            $data['medicalMaxTourists'] = number_format(round($total_capacity / PERC_TOURISTS_BUILDING['medical'], 0), 0, ',', ' ');
            $data['medicalTotalIncome'] = number_format($total_max_income, 0, ',', ' ');
        }
        return $data;
    }
