            ...
            if ($building_data->num_rows() > 0) {                // the generic building exists in the DB (always!)
                ...
                if ($count_this_building_level_under_construction == '1'){              // If the current level is under construction
                    ...
                    if ($time_left_value <= '0'){                                   // If there is no time left (building finished)
                        ...
                        $data['pre_building_time'][$i] = '<a href="'.base_url().'hotel_controller/"><div class="tooltip tooltip-bottom" data-tip="'.$this->lang->line('home')['wait_tooltip'].'">';   // For toolpit (pre)
                ...
                if ($count_this_building_under_construction == 0) { // If the building is not under construction nor upgrade
                    if ($i == '1'){         // for the first level, we allow the BUILD button
                        $data['buildingButton'][$i] = '<td><a href="'.base_url().'hotel_controller/build_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['build'].'</button></a></td>';
                    }
                      ...
                      if ($count_num_building_previous_level == '0') {      // If the previous one is NOT built, we DISABLE upgrade
                            ...
                            $data['buildingButton'][$i] = '<td><a href="'.base_url().'hotel_controller/upgrade_building/'.$currentResortID.'/'.$building_type.'/'.$i.'"><button class="btn btn-success">'.$this->lang->line('building')['upgrade'].'</button></a></td>';  
                        }  
                    }
                }
